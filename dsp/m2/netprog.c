...
/*
void signal_helper(int client_socket[30]){
    for (i = 0; i < 30; i++){ 
        if( client_socket[i] != 0 )   
        {   
            send(client_socket[i], "Shutting down, goodbye", 23, 0 );   
    ...
    {   
        ...
        //  char quit[] = "#quit";
            ...
            if (FD_ISSET( sd , &readfds)) {   
                //Check if it was for closing , an5d also read the  
                //incoming message  
                ...
                // task 3 - #say
                    ...
                    for (i = 0; i < max_clients; i++){ 
                        if( client_socket[i] != 0 )   
                        {   
                            send(client_socket[i], newbuf, strlen(buffer) - 5, 0 );   
                        }   
                    } 
                }
